...
//  dr=dr-fmod(dr,1.0); //r`j b nphchm`ke - me jnlohkhpserq
  ...
  for ( i=0; (i<n) && (fabs(y[i]) < 0.5) ; i++ );
	 if ( i>=n ) {
		xyd(&x,m,y,n);
		dr=x*mne;
		//dd=dr-fmod(dr,1.0);//r`j b nphchm`ke - me jnlohkhpserq
 		dd=(dr>0)?floor(dr):ceil(dr);
		dr=dd/nexp;
//		dd=dd-dr+fmod(dr,1.0);//r`j b nphchm`ke - me jnlohkhpserq
